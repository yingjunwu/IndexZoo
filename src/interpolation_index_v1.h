...
public:
  ...
  virtual void find(const KeyT &key, std::vector<Uint64> &values) final {
    ...
    bool is_first_match_ = true;
    
    // if the guess is correct
    if (container_[guess].key_ == key) {

      // record guess distance
      if (stats_.find_op_count_ % 1000 == 0) {
        if (is_first_match_ == true) {
          stats_.find_op_guess_distance_ += std::abs(origin_guess - guess);
          stats_.find_op_profile_count_ += 1;
